...
| [`@octanejs/react-error-boundary`](#octanejsreact-error-boundary) | `react-error-boundary@6.1.5` | Complete against the published react-error-boundary 6.1.5 function/type surface adapted to Octane: ErrorBoundary, ErrorBoundaryContext, getErrorMessage, fallback variants, onError/onReset callbacks, resetKeys, useErrorBoundary (including error), withErrorBoundary, OnErrorCallback, and UseErrorBoundaryApi. Imperative boundary refs expose resetErrorBoundary; wrapped component refs remain typed. | Component stack information is currently an empty string because Octane does not expose a public component-stack formatter; Event-handler and asynchronous errors must be passed to useErrorBoundary().showBoundary(), matching upstream's explicit forwarding requirement; Server rendering that must match upstream error propagation uses the explicit @octanejs/react-error-boundary/server entry | The explicit server entry propagates render failures like React and preserves the client boundary structure for hydration. | 2026-09-13 |
...
| [`@octanejs/tanstack-ai`](#octanejstanstack-ai) | `@tanstack/ai-react@0.24.1` | All three upstream entry points: chat, realtime, generation, media, persistence, BYOK and WebMCP hooks at the root; typed chat UI factories and widgets at ./ui; MCPAppResource at ./mcp-apps. Uses unchanged @tanstack/ai 0.54.0 and @tanstack/ai-client 0.31.1. | Octane uses native input events for built-in text controls; the public ChatInput render-prop callback retains its upstream onChange name; Octane has no StrictMode double-invoke and always provides useId; TanStack AI Devtools identifies this binding as framework: octane; Realtime reconnects and token refreshes use the latest getToken and adapter, and onStatusChange calls the latest supplied callback; Changing useChat connection or fetcher preserves the ChatClient and conversation. The replacement transport is applied before the next explicit idle sendMessage, append, or reload, preserving in-flight requests and hydration resumes; MCPAppResource uses an authored Octane host with the framework-neutral MCP Apps SDK. The React-only @mcp-ui/client is used solely as a differential test oracle | Initial chat snapshots and chat widgets render through octane/server; hydration preserves existing DOM and subsequent thread changes. | 2026-09-12 |
| [`@octanejs/tanstack-db`](#octanejstanstack-db) | `@tanstack/react-db@0.3.8` | Complete React DB adapter over imported @tanstack/db@0.9.0: five hooks, database provider/context, hydration boundary, derived query identity, explicit query keys and awaitable pagination. | Octane uses compiler-assigned hook slots and OctaneNode children; React StrictMode development double invocation is inapplicable; Warning callsites exclude Octane compiler bridge frames | Real server/client fixture exercises hydration adoption, live data and provider replacement; the upstream provider and hydration suites also run. | 2026-09-12 |
...
| [`@octanejs/tanstack-router`](#octanejstanstack-router) | `@tanstack/react-router@1.170.35` | Octane's TanStack Router binding: typed route factories and hooks, the full Match pipeline and lifecycle, file routes with TSRX-aware generator integration, full Link navigation/preloading/masking behavior, blocking, Await/deferred hydration, scroll restoration, lazy routes, not-found handling, document/head assets, and client/server SSR entries. | Refs are props — `createLink`'s `forwardRef` becomes a `ref` prop; Link callbacks receive native DOM events rather than React synthetic events; Router devtools are distributed separately | Full-document buffered and readable-stream SSR through `./ssr/server`, client hydration through `./ssr/client`, route-owned head/scripts, CSP nonce propagation, per-route SSR modes, and native Octane stream injection; covered by Octane-only framework-contract tests in ordinary shards (not a React SSR oracle). | 2026-09-12 |
| [`@octanejs/tanstack-router-ssr-query`](#octanejstanstack-router-ssr-query) | `@tanstack/react-router-ssr-query@1.167.2` | Native query-client wrapping and router SSR integration for Options and setupRouterSsrQueryIntegration. The metadata-only upstream ./package.json subpath remains omitted. | none known | Supported — this package IS the SSR integration (dehydrates query state into the router stream and wraps the app in the query provider). | 2026-09-12 |
...
[`packages/jotai`](../packages/jotai) `0.1.49` — ports `jotai@3.0.0`. Status data: [`packages/jotai/status.json`](../packages/jotai/status.json).
...
[`packages/mantine-hooks`](../packages/mantine-hooks) `0.1.33` — ports `@mantine/hooks@9.6.1`. Status data: [`packages/mantine-hooks/status.json`](../packages/mantine-hooks/status.json).

Complete @mantine/hooks 9.6.1 runtime export surface: state, timing, storage, viewport, input, focus, pointer, observer, hotkey, scrolling, collapse, drag, splitter, mask, and utility hooks.
...
Scope/evidence last checked: 2026-09-13.

- Complete pinned upstream coverage: 63 files and 545 cases per runtime lane; 223-export public type assertions; client and SSR hydration in Chromium.
- Floating-window callbacks and enabled state now use independent native refs; touch cancellation, late-ref subscription, zero-coordinate handling and body-selection restoration are preserved.
...
[`packages/octane-is`](../packages/octane-is) `0.0.1` — ports `react-is@19.3.0`. Status data: [`packages/octane-is/status.json`](../packages/octane-is/status.json).

All 26 published react-is 19.3.0 exports over Octane element descriptors and callable component metadata.
...
[`packages/react-error-boundary`](../packages/react-error-boundary) `0.1.33` — ports `react-error-boundary@6.1.5`. Status data: [`packages/react-error-boundary/status.json`](../packages/react-error-boundary/status.json).

Complete against the published react-error-boundary 6.1.5 function/type surface adapted to Octane: ErrorBoundary, ErrorBoundaryContext, getErrorMessage, fallback variants, onError/onReset callbacks, resetKeys, useErrorBoundary (including error), withErrorBoundary, OnErrorCallback, and UseErrorBoundaryApi. Imperative boundary refs expose resetErrorBoundary; wrapped component refs remain typed.
...
SSR / hydration: The explicit server entry propagates render failures like React and preserves the client boundary structure for hydration.
...
[`packages/tanstack-ai`](../packages/tanstack-ai) `0.0.46` — ports `@tanstack/ai-react@0.24.1`. Status data: [`packages/tanstack-ai/status.json`](../packages/tanstack-ai/status.json).

All three upstream entry points: chat, realtime, generation, media, persistence, BYOK and WebMCP hooks at the root; typed chat UI factories and widgets at ./ui; MCPAppResource at ./mcp-apps. Uses unchanged @tanstack/ai 0.54.0 and @tanstack/ai-client 0.31.1.
...
SSR / hydration: Initial chat snapshots and chat widgets render through octane/server; hydration preserves existing DOM and subsequent thread changes.

Scope/evidence last checked: 2026-09-12.

- All 235 pinned upstream registrations execute unchanged and as one-for-one Octane adaptations (218 client, 17 SSR), with no omitted cases.
- All 25 upstream type cases are checked with TypeScript 7 and adapted with tsrx-tsc; 123 published exports across three entry points have literal type probes.
- Differential fixtures compare streamed chat and the real MCP Apps protocol with the pinned React implementation. Native tests cover input, portals, Suspense, errors, hydration, identity, and teardown.
- Source is published as TypeScript and TSRX without declaration companions; consumers use the Octane compiler and tsrx-tsc.

## @octanejs/tanstack-db

[`packages/tanstack-db`](../packages/tanstack-db) `0.0.15` — ports `@tanstack/react-db@0.3.8`. Status data: [`packages/tanstack-db/status.json`](../packages/tanstack-db/status.json).

Complete React DB adapter over imported @tanstack/db@0.9.0: five hooks, database provider/context, hydration boundary, derived query identity, explicit query keys and awaitable pagination.

Known divergences:

- Octane uses compiler-assigned hook slots and OctaneNode children.
- React StrictMode development double invocation is inapplicable.
- Warning callsites exclude Octane compiler bridge frames.

SSR / hydration: Real server/client fixture exercises hydration adoption, live data and provider replacement; the upstream provider and hydration suites also run.

Scope/evidence last checked: 2026-09-12.

- 161 pristine runtime cases, 160 applicable adapted cases and 18 pristine/adapted type registrations are crosswalked.
- Legacy native scenarios remain, alongside real React differential and native hydration tests.
- All ten required parity lanes and the complete machine evidence matrix pass, including strict packed Node and browser source consumers.
...
[`packages/tanstack-router`](../packages/tanstack-router) `0.1.53` — ports `@tanstack/react-router@1.170.35`. Status data: [`packages/tanstack-router/status.json`](../packages/tanstack-router/status.json).
...
- The framework-neutral runtime dependency is `@tanstack/router-core@1.171.29`.
...
- Immutable release source, 986 runnable upstream cases plus one authenticated upstream skip, all 167 upstream type cases, and all nine required parity lanes pass. All 629 public export contracts and strict packed Node/browser consumers pass.
- Native scroll-script hydration retains DOM identity; the client SSR runtime uses exact browser-safe declarations without requiring Node ambient types.

See also: [`docs/tanstack-parity-audit.md`](tanstack-parity-audit.md)

## @octanejs/tanstack-router-ssr-query

[`packages/tanstack-router-ssr-query`](../packages/tanstack-router-ssr-query) `0.0.43` — ports `@tanstack/react-router-ssr-query@1.167.2`. Status data: [`packages/tanstack-router-ssr-query/status.json`](../packages/tanstack-router-ssr-query/status.json).

Native query-client wrapping and router SSR integration for Options and setupRouterSsrQueryIntegration. The metadata-only upstream ./package.json subpath remains omitted.
...
- All seven required parity lanes, strict source and public type programs, and packed Node/browser consumers pass. The published upstream adapter has no dedicated runtime or type-test registrations.
- Native coverage exercises cached and streamed queries, provider hydration, retained input edits and focus, portals, client isolation and observer cleanup.
...
[`packages/window`](../packages/window) `0.0.19` — ports `react-window@2.3.1`. Status data: [`packages/window/status.json`](../packages/window/status.json).

List, Grid, getScrollbarSize, useDynamicRowHeight, four imperative-ref hooks, and all eight public types from react-window 2.3.1. The complete 119-case upstream unit suite runs in pristine React and generated Octane lanes, with differential, SSR, hydration, shared public types, and native Chromium startup, scrolling, and direction-change coverage.
...
- Targets react-window v2.3.1. Legacy v1 FixedSizeList, VariableSizeList, FixedSizeGrid, and VariableSizeGrid APIs are absent from the pinned upstream package.
- All 125 upstream registrations are accounted for: 119 paired unit cases, four native browser conformance cases, and two inapplicable React Server Components scenarios. Browser validation covers Chromium.
