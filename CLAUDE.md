...
## 프로젝트 개요
복잡한 화면 렌더링 과정을 기계어로 사전 컴파일하여 눈 깜짝할 사이에 초고속으로 화면을 띄워주는 "차세대 초광속 웹 렌더링 엔진"
스마트폰이나 저사양 컴퓨터에서도 버벅거림 없이 120프레임의 부드럽고 매끄러운 웹 사용자 경험을 보장
단 1밀리초의 화면 지연도 허용하지 않는 인터랙티브 웹 앱과 고성능 대시보드를 위한 초고속 터보 엔진
